...
it('returns an error if the ticket doesn not exist', async () => {
  const ticketId = new Types.ObjectId().toString();
  await request(app).post('/api/orders').set('Cookie', global.signin()).send({ ticketId: ticketId }).expect(404);
});
